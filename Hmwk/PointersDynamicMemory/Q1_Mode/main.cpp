...
void minPos(int *array, int n, int pos){
    for(int i = pos + 1; i < n; i++){
        if(*(array+pos)>*(array+i)) swap(array+pos, array+i);
    }
}
...
int *mode(const int *array, int arySize){
    ...
    for(int i = 1; i < arySize; ++i){
        (ary[i] == ary[i-1]) ? curFreq++ 
                             : (maxFreq = (curFreq > maxFreq ? curFreq : maxFreq), curFreq = 1);
    }
    ...
    if(maxFreq > 1){
        ...
        for(int i = 1; i < arySize; ++i){
            (ary[i] == ary[i-1]) ? curFreq++ 
                                 : (nmodes += (curFreq == maxFreq ? 1 : 0), curFreq = 1);
        }
        nmodes += (curFreq == maxFreq ? 1 : 0);
    }
    
    //Allocate the mode array
    //Again this is just a stub.
    int *modeAry = new int[nmodes+2];
    modeAry[0] = nmodes;         // number of modes
    modeAry[1] = maxFreq;        // frequency of modes
    ...
    if(maxFreq > 1){
        for(int i = 1; i < arySize; ++i){
            (ary[i] == ary[i-1]) ? curFreq++ 
                                 : (idx += (curFreq == maxFreq ? (modeAry[idx] = ary[i-1], 1) : 0), curFreq = 1);
        }
        idx += (curFreq == maxFreq ? (modeAry[idx] = ary[arySize-1], 1) : 0);
    }
